INSERT INTO news
  (section, title, summary, body, image_url, status, author_role, author_name, published_at, created_at, updated_at)
SELECT
  'terkini', 'Program Dakwah Semarang Terus Bertumbuh',
...
WHERE NOT EXISTS (
  SELECT 1 FROM news WHERE section = 'terkini'
);

INSERT INTO study_articles
  (section, title, summary, body, image_url, status, author_role, author_name, published_at, created_at, updated_at)
SELECT
  'artikel-kajian', 'Menjaga Keikhlasan dalam Berdakwah',
...
WHERE NOT EXISTS (
  SELECT 1 FROM study_articles WHERE section = 'artikel-kajian'
);
